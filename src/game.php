...
if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    if (isset($_SESSION['player'])){
        if (isset($_POST['selectedPlant'])){
            ...
            $currentPlanet = $player->getPlanet($_SESSION['currentPlanet']);
            if($player == FALSE || $currentPlanet == FALSE){
                die("Something went wrong with unserialising");
            }
            $selectedPlant = $currentPlanet->plants[$_POST['selectedPlant']];
            
            
            $_SESSION['player'] = serialize($player);
            $_SESSION['currentPlanet'] = $currentPlanet->name;
            $_SESSION['selectedPlant'] = $_POST['selectedPlant'];
            
        }elseif (isset($_POST['plant'])) {
            $player = unserialize($_SESSION['player']);
            $currentPlanet = $player->getPlanet($_SESSION['currentPlanet']);
            $currentPlanet->plants[$_SESSION['selectedPlant']]->isPlanted = true;
            echo $currentPlanet->name;
            $selectedPlant=$currentPlanet->plants[$_SESSION['selectedPlant']];
            if($player == FALSE || $currentPlanet == FALSE){
                die("Something went wrong with unserialising");
            }
            $_SESSION['player'] = serialize($player);
        ...
        echo "Player doesn't have a session yet.";
        // Creating a planet
        ...
        //Select a random plant from the ones available to the player
        $selectedPlant = $currentPlanet->plants[0];
        $player = new Player(session_id(), 1, $_POST['name'], array($currentPlanet->name =>$currentPlanet));
        $_SESSION['player'] = serialize($player);
        $_SESSION['currentPlanet'] = $currentPlanet->name;
        $_SESSION['selectedPlant'] = 0;
    }
}
?>
<div class="container">
    <div class="plant-info">
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="game.php" method="POST">
                                <input type="hidden" name="selectedPlant" value=<?= $key?>>
        ...
        <div class="plant-detail">
            ...
            <?php if ($selectedPlant->isPlanted) : ?>
                <span>Already Planted!</span>
            <?php else : ?>
                <form action="game.php" method="POST">
                    <input type="hidden" name="plant" value=<?= $key?>>
